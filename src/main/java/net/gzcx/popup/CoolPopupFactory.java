...
import net.gzcx.widget.NinePatchBgPanel;
...
public class CoolPopupFactory extends PopupFactory {
     ...
     * 透明背景的Popup实现类.
     *
     * <p>本类的大部分代码都是原封不动地参考自Popup父类(JDK版本是1.6_u18版的源码)，因为Popup父
     * 类中的多数方法都是使用的包内可见性，所以没法直接继承，像这种不允许第3方自行扩展的情 况在Swing标准包里比较少见，不知原作者是出于何种考虑，无疑这样的类实现是相当的欠缺远
     * 见性，但这种情况在LNF里很常见。
     */
    protected class TranslucentPopup extends Popup {
        ...
        // copy all from parent class
        /**
         * Creates a <code>Popup</code> for the Component <code>owner</code> containing the
         * Component <code>contents</code>. <code>owner</code> is used to determine which <code>
         * Window</code> the new <code>Popup</code> will parent the <code>Component</code> the
         * <code>Popup</code> creates to. A null <code>owner</code> implies there is no valid
         * parent. <code>x</code> and <code>y</code> specify the preferred initial location to place
         * the <code>Popup</code> at. Based on screen size, or other paramaters, the <code>Popup
         * </code> may not display at <code>x</code> and <code>y</code>.
         *
        ...
        /**
         * Makes the <code>Popup</code> visible. If the <code>Popup</code> is currently visible,
        ...
        // copy all from parent class
        /**
         * Hides and disposes of the <code>Popup</code>. Once a <code>Popup</code> has been disposed
         * you should no longer invoke methods on it. A <code>dispose</code>d <code>Popup</code> may
         * be reclaimed and later used based on the <code>PopupFactory</code>. As such, if you
         * invoke methods on a <code>disposed</code> <code>Popup</code>, indeterminate behavior will
         * result.
         */
        public void hide() {
            Component component = getComponent();
            ...
            if (c instanceof JWindow) {
                ...
                com.sun.awt.AWTUtilities.setWindowOpacity(
                        ...
                        isTooltip
                                ...
                                : isComboBoxPopup
                                        ? 0.95f
                                        : 0.95f); // 0.85f : 0.95f);//0.8f : 0.95f);
                //				WindowTranslucencyHelper.setOpacity(component,
                //						isTooltip ? 1.0f : isComboBoxPopup?0.95f : 0.95f);//0.85f : 0.95f);//0.8f :
                // 0.95f);

                // 图片填充背景的内容面板
                NinePatchBgPanel imageContentPane =
                        new NinePatchBgPanel()
                                .setN9(
                                        isTooltip
                                                ? NPIconFactory.getInstance().getTooltipBg()
                                                : isComboBoxPopup
                                                        ? // 是下拉框列表则使一个跟JScrollPane一样的背景图，好看！
                                                        NPIconFactory.getInstance()
                            ...
                            .setBorder(
                                    isTooltip
                                            ? BorderFactory.createEmptyBorder(
                                                    107, 27, 12, 27) // 6,8,12,12)
                                            : isComboBoxPopup
                                                    ? BorderFactory.createEmptyBorder(6, 4, 6, 4)
                                                    : BorderFactory.createEmptyBorder(
                                                            5, 3, 6,
                                                            3)); // 10,2,10,4));//5,5,20,5));
                }
                ...
                // pop的背景撒）
                if (isComboBoxPopup) // contents instanceof BasicComboPopup)//* add by jb2011
                                     // 2012-08-31
                {
                    ...
                    if (cs != null && cs.length > 0) {
                        ...
                        for (Component com : cs) {
                            if (com instanceof JScrollPane) ((JScrollPane) com).setOpaque(false);
                        }
                    }
                    //					BEUtils.componentsOpaque(((BasicComboPopup)contents).getComponents(),
                    // false);
                }
                // 如果是菜单父宿主则所有JMenuItem也设置透明,jb2011 2009-08-29
                    ...
                    for (int i = 0; i < mes.length; i++) {
                        if (mes[i] instanceof JMenuItem) ((JMenuItem) mes[i]).setOpaque(false);
                    }
                }
        ...
        // copy all from parent class
        /**
         * Causes the <code>Popup</code> to be sized to fit the preferred size of the <code>
         * Component</code> it contains.
         */
        protected void pack() {
            Component component = getComponent();
        ...
        // copy all from parent class
        /**
         * Returns the <code>Window</code> to use as the parent of the <code>Window</code> created
         * for the <code>Popup</code>. This creates a new <code>DefaultFrame</code>, if necessary.
         *
         * @param owner the owner
        ...
        // copy all from parent class
        /**
         * Creates the Component to use as the parent of the <code>Popup</code>. The default
         * implementation creates a <code>Window</code>, subclasses should override.
         *
         * @param owner the owner
        ...
        // copy all from parent class
        /**
         * Returns the <code>Component</code> returned from <code>createComponent</code> that will
         * hold the <code>Popup</code>.
         *
         * @return the component
         */
         ...
         *
         * <p>PopupFactory uses this as a way to know when the Popup shouldn't be cached based on
         * the Window.
         */
