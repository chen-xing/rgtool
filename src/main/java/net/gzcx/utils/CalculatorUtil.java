...
public class CalculatorUtil {
    ...
    public static final Map<Character, Integer> symLvMap =
            new HashMap<Character, Integer>(); // 符号优先级map
    ...
    /**
     * @param str 表达式
     * @return true表达式正确，false表达式错误 @Title: checkFormat @Desc: 检查表达式格式是否正确
     */
        ...
        for (int i = 1; i < str.length() - 1; i++) {
            ...
            if (!(isCharNum(c))) {
                if (c == '-' || c == '+' || c == '*' || c == '/') {
                    ...
                    if (!(isCharNum(str.charAt(i - 1))
                            || str.charAt(i - 1) == ')')) { // 若符号前一个不是数字或者“）”时
                        return false;
                    }
                }
                if (c == '.') {
                    if (!isCharNum(str.charAt(i - 1))
                            || !isCharNum(str.charAt(i + 1))) { // 校验“.”的前后是否位数字
                        return false;
                    }
                }
            }
    ...
    /**
     * @param str
     * @return 标准表达式 @Title: change2StandardFormat @Desc:
    ...
    /**
     * @param str
     * @return 参数 @Title: isBracketCouple @Desc: 校验括号是否配对
     */
    ...
    /**
     * @param str 计算结果
     * @return 规范的计算结果 @Title: formatResult @Desc: 处理计算结果的显示
     */
    ...
    /**
     * @param c
     * @return 参数 @Title: isCorrectChar @Desc: 校验字符是否合法
     */
    ...
    /**
     * @param c
     * @return @Title: isCharNum @Desc: 判断是否为数字
     */
     ...
     * @param num1
     * @param num2
     * @return 计算结果 @Title: reduce @Desc: 减
     */
     ...
     * @param num1
     * @param num2
     * @return 计算结果 @Title: multiply @Desc: 乘
     */
     ...
     * @param num1
     * @param num2
     * @return 计算结果 @Title: divide @Desc: 除
     */
