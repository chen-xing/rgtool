...
/** @author lonely @Date 2019/9/2 14:05 @Description 基于okhttp的工具类 */
@Slf4j
public class OkHttpUtil {
    ...
    public Response postData(
            String url, Map<String, Object> bodyParams, Map<String, String> headerMap)
            throws Exception {
        // 1构造RequestBody
        ...
        // Request request = requestBuilder.post(body).addHeader(CONTENT_TYPE,
        // HttpContentTypeEnum.JSON.contentTypeValue).url(url).build();
        // 3 将Request封装为Call
        Call call = mOkHttpClient.newCall(request);
        // 4 执行Call，得到response
        Response response = null;
    ...
    private RequestBody setRequestBody(
            Map<String, Object> bodyParams, Map<String, String> headerMap) throws Exception {
        ...
        if ("application/x-www-form-urlencoded".equals(contentType)) {
            ...
            // json
            return buildRequestBodyByJson(new Gson().toJson(bodyParams));
        }
    }
    ...
    public void postJsonAsyn(String url, String json, final MyNetCall myNetCall)
            throws IOException {
        RequestBody body = RequestBody.create(JSON, json);
        // 2 构造Request
        Request.Builder requestBuilder = new Request.Builder();
    ...
    private static Request.Builder addHeaders(
            Map<String, String> headerMap, Request.Builder builder) {
        if (headerMap != null && !headerMap.isEmpty()) {
