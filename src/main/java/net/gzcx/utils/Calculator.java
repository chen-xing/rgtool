...
public class Calculator {
     ...
     * @param symStr 符号串
     * @param numLst 数字集合
     * @return 计算结果 @Title: doCalculate @Desc: 计算
     */
    public Double doCalculate(String symStr, List<Double> numLst) throws Exception {
        ...
        while (symStack.isEmpty()
            ...
            if (CalculatorUtil.symLvMap.get(symStr.charAt(j))
                    > CalculatorUtil.symLvMap.get(symStack.getLast())) { // 比较符号优先级，若当前符号优先级大于前一个则压栈
                if (symStr.charAt(j) == '(') {
